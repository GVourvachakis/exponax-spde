...
def spatial_norm(
    ...
    *,
    ...
    See [`exponax_spde.metrics.spatial_aggregator`][] for more details. This function
    sums over the channel axis **after aggregation**. If you need more low-level
    control, consider using [`exponax_spde.metrics.spatial_aggregator`][] directly.
...
def RMSE(
    ...
    *,
    ...
    using [`exponax_spde.metrics.spatial_aggregator`][] directly.
...
def nRMSE(
    ...
    *,
    ...
    [`exponax_spde.metrics.spatial_aggregator`][] directly.
    ...
    **Arguments:**
    ...
    - `u_pred`: The state array, must follow the `exponax_spde` convention with a
        leading channel axis, and either one, two, or three subsequent spatial
        axes.
    - `u_ref`: The reference state array. Must have the same shape as `u_pred`.
    - `domain_extent`: The extent `L` of the domain `Ω = (0, L)ᴰ`. Must be
